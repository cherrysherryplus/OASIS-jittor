...
class OASIS_Generator(nn.Module):
    def __init__(self, opt):
        ...
        self.conv_img_1 = nn.Conv2d(ch*8, 3, 3, padding=1)
        self.conv_img_2 = nn.Conv2d(self.channels[-1], 3, 3, padding=1)
...
class StripPooling(nn.Module):
    ...
    def execute(self, x):
        ...
        x1 = self.conv2_5(jt.nn.relu(x2_1 + x2_2 + x2_3))
        x2 = self.conv2_6(jt.nn.relu(x2_5 + x2_4))
        out = self.conv3(jt.concat([x1, x2], dim=1))

        return jt.nn.relu(jt.concat((x, out), 1))